...
from utils import API, Artist, Album, Song
...
class APIUser:  # we use this to interact with the MobileClient
    ...
    def search(query):
        ...
        return {'artists': [Artist(API.get_artist_info(artist['artist']['artistId'])) for artist in
                            query_results['artist_hits']],
                'albums': [Album(API.get_album_info(album['album']['albumId'])) for album in
                           query_results['album_hits']],
                'songs': [Song(API.get_track_info(song['track']['storeId'])) for song in
