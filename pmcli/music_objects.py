...
class MusicObject(abc.ABC):  # albums, artists, and songs
    ...
    @abc.abstractmethod
    def length(self):
        # return the summed length of all of an item's sublists + itself
        pass
...
class Artist(MusicObject):
    def __init__(self, artist):
        # artist is a dict from Mobileclient.get_artist_info
        self.contents = {}  # the artist's albums and songs

        # some ugly checks because some artists don't have albums and/or songs
        if 'albums' in artist:
            # make this a list even though it's only one
            # item to make other methods simpler
        ...
        if 'topTracks' in artist:
            self.contents['songs'] = [
                {'name': song['title'], 'id': song['storeId']}
    ...
    def length(self):  # return number of albums and songs and itself
        return 1 + sum([len(self.contents[key]) for key in self.contents])
    ...
    def play(self):  # play top tracks
        ...
        for song in self.contents['songs']:
            ...
            print('(%d/%d): %s' % (
                i, len(self.contents['songs']),
                ' - '.join((song['name'], api_user.API.get_track_info(
                    song['id'])['album']))))

            # we set the exit code for 'q' to 11 in our custom input.conf
            if subprocess.call([
                    'mpv', '--really-quiet',
                    '--input-conf=~/.config/pmcli/mpv_input.conf', url
            ]) is 11:
                break
            i += 1

    def show(self):  # print artist name, albums, and songs
        i = 1
...
class Album(MusicObject):
    def __init__(self, album):
        # album is a dict from Mobileclient.get_album_info
        # contents: the album's artist and songs
        self.contents = {
            'artist': [{'name': album['artist'], 'id': album['artistId'][0]}],
            'songs': [{'name': song['title'], 'id': song['storeId']}
    ...
    def play(self):  # play the album's songs
        ...
        for song in self.contents['songs']:
            ...
            print('(%d/%d): %s' % (
                (i, len(self.contents['songs']),
                 ' - '.join((self.contents['artist'][0]['name'], song['name']))
                 )))
...
class Song(MusicObject):
    def __init__(self, song):
        # song is a dict from Mobileclient.get_track_info
        # contents: the song's artist and album
        self.contents = {
            'artist': {'name': song['artist'], 'id': song['artistId'][0]},
            'album': {'name': song['album'], 'id': song['albumId']}
        }

        super().__init__(song['title'], song['storeId'])

    def length(self):  # length of 1 song is always just 1
        return 1

    def to_string(self):  # return artist - song - album
        return ' - '.join(
            (self.contents['artist']['name'],
             self.name, self.contents['album']['name']))
